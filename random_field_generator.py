...
def generate_random_field_mlmc(x_l,xl_1,x_s,k,g0,gN)
    ...
    for i in range(s):
        ...
        # ASSEMBLE M_e^l IS NON TRIVIAL!!! -> implement a suitable test function in auxliary fem
        M_e = np.array([M_e_l,M_e_l_l_1],[M_e_l_l_1.T, M_e_l_1])
        b_e = np.random.multivariate_normal(mean=np.zeros(4),cov=M_e)
        b += #Suitable Global To local Matrix
