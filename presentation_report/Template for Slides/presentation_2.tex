...
\bigskip
\item While the desired mean can be achieved via the summation of the mean vector, the right covariance is quite more involved since it is necessary an expensive \textbf{Cholesky factorization}.
\end{itemize}
...
\bigskip
A clever way of doing that is choose as FEM space an extension of FEM space 
that we use on $G$ to solve the original PDE with random coefficient, or vice versa, choose the FEM on G as a restriction of the one define on D.

\bigskip
Doing so we obtain for free the value of the random field on the space spacial nodes where we will solve the original problem.
\bigskip
...
\bigskip
The discretised weak form of the PSDEreads:
\begin{equation}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{images/supermesh.png}
...
\bigskip
In practice for Lagrange FEM:
\begin{equation}
  b_e^k = (R_e^k)^T H_r\, |e_r|^{-1/2} |e|^{1/2}\, \mathbf{z}_e,
  \quad \mathbf{z}_e \sim \mathcal{N}(0,I),\quad k \in \{l-1,\,l\}
\end{equation}
where $M_e^S / |e| = H_r H_r^T / |e_r| = \mathrm{const}$ is the Cholesky factorisation
over the reference element $e_r$. $H_r$ is computed \textbf{only once}.
...
\begin{frame}{1D Numerical Experiments}

We aim to replicate the results shown in the article in the \textbf{simpler 1D case}. This
allows us to focus on the main ideas rather than the more complex FEM reasoning which is
beyond the scope of the seminar.

\bigskip
\textbf{Setup:}
\begin{itemize}
  \item Domains: $G = [-0.5,\,0.5]$ and $ D = [-1,\,1]$
  \item FEM function space: piecewise linear test function (Lemma holds).
  \item Mat\'ern covariance with $\kappa=10$.
  \item Quantity of interest: $\mathcal{P} \approx \|q_h\|_{L^2(G)}$, estimated
    via Monte Carlo.
  \item $N = 100$, $N = 1\,000$ and  $N = 10\,000$ 
  \item \href{https://github.com/fede-mat/mlmc_fem_1d_solver}{\faGithub\ mlmc\_fem\_1d\_solver}
\end{itemize}
...
\begin{frame}{Summary}
  \begin{itemize}
    \item[\textbullet] Sampling a Mat\'ern random field via the SPDE approach reduces to
      sampling a Gaussian vector with \textbf{mass matrix covariance}, avoiding dense
      covariance factorisation ($\mathcal{O}(m^3) \to \mathcal{O}(m_e^3)$).\bigskip

    \item[\textbullet] The global mass matrix assembles from local element matrices, enabling
      \textbf{element-wise Cholesky factorisation} and embarrassingly parallel sampling;
      for Lagrange elements a single reference factorisation suffices.\bigskip

    \item[\textbullet] MLMC variance reduction requires \textbf{coupled sampling} of
      $u_{h_l}$ and $u_{h_{l-1}}$ from the same white noise realisation; for non-nested
      meshes this is achieved via a \textbf{super-mesh} and local interpolation matrices
      $R_e^l$, $R_e^{l-1}$.\bigskip

    \item[\textbullet] The method is effective but overall efficiency depends on available
      \textbf{parallel hardware}, since each sample requires solving two PDEs.
  \end{itemize}
\end{frame}
